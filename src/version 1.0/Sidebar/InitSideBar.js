...
var mySingletonSideBar = (function() {
    ...
    function init(sideBarCategories, sideBarSubCategories, canvas) {
        sidebarDiv = $("#sideBar");
        //$("#sideBar").left($("#sideBarContainer").width()/2);
        //$("#sideBar").left(0);
        $("#sideBar").css('width',220);
        $("#sideBar").css('left',$("#sideBarContainer").width()/2-$("#sideBar").width()/2);
            ...
            if (image) {
                if(canvas){
                    ...
                    if (canvas.elementsUnderDragElement.length == 1) {
                        ...
                        if (canvas.elementsUnderDragElement[0].id === "groupLogicExprError") 
                        {
                            canvas.elementsUnderDragElement.length = 0;  
                        }
                        else {
                            AddDoNothingElement(canvas.elementsUnderDragElement[0].left, canvas.elementsUnderDragElement[0].top, canvas, 4, false);
                            canvas.makeIntersection = true;
                            canvas.objectIntersected = canvas.elementsUnderDragElement[0];
                            canvas.renderAll();
                        }
                    }
                }
